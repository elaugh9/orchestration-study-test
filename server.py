...
if __name__ == '__main__':
    PORT = int(os.environ.get("PORT", 8000))  # Use Heroku's dynamic port or default to 8000

    # Load data file from an environment variable or default path
    data_pth = os.getenv("DATA_PATH", "server_data_final_maths.json")
    
    # Load the server data
    with open(data_pth, "r") as f:
        server_data = json.load(f)

    # Process the server data (embeddings, examples per batch, etc.)
    ...
    # Setup the handler and start the server
    handler = partial(ServerHandler, None, emb_data, examples_per_batch, arm_list, num_classes, n_trials)
    with HTTPServer(('0.0.0.0', PORT), handler) as httpd:
        try:
            ...
            httpd.server_close()  # Close the server after interruption
