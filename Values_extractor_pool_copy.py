        ...
        while start_index < len(df):
            try:
                ...
                if start_index + 1 < len(df):
                    ...
                    llm_outsourcing_overridden = df.loc[start_index + 3, "LLM_outsourcing_overridden"]
                    human_outsourcing_overridden = df.loc[start_index + 3, "human_outsourcing_overridden"]
                    ai_orch_agreement = df.loc[start_index + 3, "ai_orch_agreement"]
                    human_orch_agreement = df.loc[start_index + 3, "human_orch_agreement"]
                    self_orch_agreement = df.loc[start_index + 3, "self_orch_agreement"]
                    ...
                    incorrect_answers_by_category = df.loc[start_index + 3, "incorrect_answers_by_category"]
                    ...
                    extracted_data.append([
                        topic, llm_answer, population_answer, response, label,
                        llm_support_used, human_support_used, llm_outsourcing_overridden,
                        human_outsourcing_overridden, ai_orch_agreement, human_orch_agreement,
                        self_orch_agreement, recommendation, recommendation_lists, incorrect_answers_by_category
                    ])
